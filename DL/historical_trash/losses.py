...
def background_layers_loss(x: torch.Tensor, blocks: List[torch.Tensor], feature_extractor: nn.Module, alpha: float,
    ...
    # 1. Базовая маска: 1 - это пиксель буквы, 0 - чистый фон
    digit_mask = (x >= 0.01).float()

    # 2. Dilation (расширение) штриха, чтобы не штрафовать границы
    if n > 0:
    ...
    # 3. Пропускаем маску через пространственные преобразования сети
    for layer in feature_extractor:
        ...
        if layer_name == 'MyConv2d':
            ...
            # Пиксель на выходе имеет право активироваться, если хотя бы один
            # пиксель в его окне обзора (kernel_size) является буквой (MaxPool)
            current_mask = F.max_pool2d(current_mask, kernel_size=k, stride=s, padding=p)

            if block_idx < len(blocks):
                # Инвертируем: 1 - фон (нельзя активироваться), 0 - буква (можно)
                bg_mask = 1.0 - current_mask
                block_tensor = blocks[block_idx]

                # Усредняем штраф по пикселям, чтобы он не подавил CrossEntropy loss
